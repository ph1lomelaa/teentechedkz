...
/**
 * Роли, которые назначают руками. Это НЕ весь `MENTOR_ROLE_LABELS`: справочник
 * шире, потому что в нём живут и роли из старых данных, а осознанно назначают
 * только эти — четыре менторские роли из регламента МЗК плюс сам МЗК-менеджер.
 *
 * Один список на карточку студента и на общую базу: пока он был выписан в
...
/**
 * Кого показывать во втором списке — «кого назначить» — для выбранной роли.
 * МЗК ведёт студента целиком, и назначают на неё менеджера (UserRole.mzk_manager),
 * а не ментора; всё, чего нет в карте, читается как 'mentor'.
 *
 * Карта живёт рядом с ролями, а не на экране: списки «кого назначить» есть и в
 * общей базе, и в карточке студента, и разъехавшееся правило означало бы, что
 * одно и то же действие предлагает разных людей в зависимости от того, откуда
 * его начали.
 */
