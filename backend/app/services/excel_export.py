...
def export_login_links(rows: list[dict], skipped: list[dict]) -> bytes:
    ...
    Почему пропущенные остаются строками
    ------------------------------------
    Неактивным аккаунтам ссылку не выдаём (см. create_login_link), но со списка
    их не снимаем: строка остаётся, ссылка пуста, причина — в «Примечании».
    Иначе файл приходит на 14 строк вместо 16, и при переносе колонки со
    ссылками в общую таблицу всё ниже первого пропущенного съезжает — человек
    получает в своей графе чужой инвайт. Лист «Пропущены» остаётся сводкой:
    кого именно и почему обошли, не вычитывая основной лист.

    Email в основном листе стоит вторым не для красоты: по нему ссылки
    подтягиваются в чужую таблицу через ВПР, а не блочной вставкой.
    """
    wb = openpyxl.Workbook()
    ...
    ws.title = "Ссылки"
    _header(ws, [
        "Имя", "Email", "Роль", "Специализация", "Ссылка для входа", "Код", "Действует до",
        "Примечание",
    ])
    for row in rows:
        ws.append([
