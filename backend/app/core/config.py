...
class Settings(BaseSettings):
    ...
    # Менторам сопоставлять нечего: карточки-сущности у них нет, ментор — это
    # просто User(role=mentor). Код здесь заменяет то, чем для студента служит
    # совпадение телефона с карточкой.
    JOIN_MENTOR_CODE: str = ""
