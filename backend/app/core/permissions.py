...
# Скоуп ментора выключен (решение владельца 02.09.2026): сотрудник открывает
# карточку любого ученика. Карты оставлены пустыми, а не удалены из 20 правил:
# вернуть ограничение — значит вписать сюда одну строку, а не править реестр
# целиком. Настоящий гейт живёт в services/mentor_scope.py, здесь — то, что
# видит админ на странице прав, и расходиться им нельзя.
_MENTOR_SCOPED: dict[UserRole, Scope] = {}
#: У ученика скоуп остаётся: он видит только свою запись — это не про удобство
#: команды, а про то, что чужой кабинет ему не принадлежит.
_OWNER_SCOPED = {UserRole.student: Scope.own}
...
RULES: tuple[Rule, ...] = (
    ...
    # Полное удаление — отдельным действием от manage: оно необратимо и сносит
    # договоры, платежи и начисления менторам вместе с карточкой.
    Rule("students", Action.delete, MANAGERS,
         extra_rules=("Удаляет карточку и всё зависимое без возврата; вход "
                      "портального аккаунта закрывается — students.py:1683, student_purge.py:110",)),
    ...
    Rule("mentor_assignments", _M, STAFF,
         extra_rules=(
             "Назначить ответственным можно любого сотрудника — mentor_assignments.py:161",
    ...
    # Доска распределения: кто из сотрудников ведёт каких студентов. Право
    # шире, чем «свои назначения» (mentor_assignments), поэтому отдельное:
    # видеть чужую нагрузку целиком — задача управления, а не ментора. Править
    # доску (перетаскивание) по-прежнему решает mentor_assignments:manage.
    Rule("assignment_overview", _V, MANAGERS,
         extra_rules=(
             "Доска строится по активным назначениям, а не по договорам — "
             "mentor_assignments.py:623",
         )),
