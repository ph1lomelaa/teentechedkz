...
Ради чего тест
--------------
В `app/core/deps.py` жил второй реестр прав — `PERMISSIONS`, `ROLE_PERMISSIONS`,
`has_permission`, `require_permission`. Он прожил без пользы: `require_permission`
вызывался из пяти мест, все в `tasks.py`, а права `manage_users` и
`manage_regulations` не вызывались ниоткуда вообще (удалены в конце Этапа 1).
Остальные 354 эндпоинта завели себе по самодельному гейту. Сама вторая система
удалена 30.08.2026 — четыре операционных права переехали правилами сюда.
