...
Всё, что описано ниже как сделанное, лежит в семи коммитах этой ветки — от
`docs: план централизации прав доступа` до `refactor(api): перевести проверки
прав на реестр`. Рабочее дерево чистое, `pytest -q` даёт **390 passed**.

**Следующий шаг:** чистка мёртвого в `deps.py` (последний пункт Этапа 1),
затем чанк 2.1 — эндпоинт матрицы.
