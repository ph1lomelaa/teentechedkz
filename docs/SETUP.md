...
Как поднять проект локально, какие переменные окружения нужны, что делать, когда
что-то не работает, и как загрузить данные при первом запуске.

- Что это за система и что она умеет — [README.md](../README.md).
- Как всё устроено внутри (веб-процесс vs воркер, очереди, интеграции) —
...
├── .env.example
├── README.md                     # что за система и что умеет
